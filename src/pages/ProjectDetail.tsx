    ...
    <main className="min-h-screen bg-bg text-text">
      <section className="section-padding pt-24 md:pt-28">
        <div className="container-tight space-y-8">
          <header className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-border/65 bg-panel/55 p-4 md:p-5">
            ...
            <div className="flex flex-wrap items-center gap-2">
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="inline-flex items-center gap-2 rounded-lg border border-border/70 bg-panel/45 px-3 py-2 text-sm"
              >
          ...
          <article className="grid gap-6 lg:grid-cols-[1.15fr_0.85fr]">
            <div className="glass-card p-7">
              ...
              <p className="mt-3 text-lg text-slate-200">{project.tagline}</p>
              ...
              <div className="mt-6">
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  {project.stack.map((item) => (
                    <span key={item} className="rounded-full border border-border/70 bg-panel/45 px-3 py-1 text-xs text-muted">
                      {item}
                    </span>
                  ))}
                </div>
            ...
            <div className="glass-card overflow-hidden p-3">
              <img src={project.cover} alt={`Capa do projeto ${project.name}`} className="aspect-[4/3] w-full rounded-xl border border-border/60 object-cover" />
            </div>
          </article>
            ...
            <div className="mt-4 grid gap-4 md:grid-cols-2">
              {project.gallery.map((item) => (
                <figure key={item.src} className="space-y-2">
                  <img src={item.src} alt={item.alt} className="aspect-[16/10] w-full rounded-xl border border-border/60 object-cover" />
            ...
            <div className="grid gap-4 md:grid-cols-2">
              {otherCases.map((item) => (
                <article key={item.slug} className="glass-card p-5">
                  <img src={item.cover} alt={`Capa do projeto ${item.name}`} className="aspect-[16/9] w-full rounded-lg border border-border/60 object-cover" />
